...
Note: This profile defines how holder authentication can be proved and claim authenticity is protected on a technically level by way of requiring certain protocol and credential format features. Out of scope are concrete holder authentication mechanisms (which ensure the holder can only sign the presentation) and policies and procedures (as this is a technical interop profile and not a policy definition).
