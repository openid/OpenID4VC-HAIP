...
* For signed requests, the Verifier MUST use, and the Wallet MUST accept the Client Identifier Prefix `x509_hash` as defined in Section 5.9.3 of [@!OIDF.OID4VP]. X.509 certificate profiles for use with `x509_hash` are out of scope of this specification. Ecosystems MAY define their own X.509 certificate profiles for `x509_hash` and use them accordingly. For example, an mDL ecosystem may use the Reader Authentication Certificate profile defined in ISO/IEC 18013-5, Annex B with `x509_hash`.
